...
CARD = AgentCard(
    ...
    raw={
        ...
        "message_types": ["pemdas.add:v1"],    # what payload types this agent handles
    },
    card_sha256="",
)

SELF_ENDPOINT = make_endpoint(name=CARD.name, version=CARD.version, url=CARD.url, skill="pemdas.add")




def add_brain(text: str) -> Dict[str, Any]:
    ...
    if req["message_type"] != "pemdas.add:v1":
        raise ValueError(f"ADD cannot handle message_type={req['message_type']}")
    ...
    return build_task_result(
        artifact_name="pemdas.add.result",
        parts=[{"type": "text", "text": json.dumps(resp)}],  # optional receipt
        meta=resp,
    )


app = create_a2a_app(CARD, add_brain)
