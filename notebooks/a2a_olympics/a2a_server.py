...
def create_a2a_app(agent, brain: Callable[[str], Any]) -> FastAPI:
    ...
    async def a2a(request: Request) -> Dict[str, Any]:
        ...
        text = extract_user_text(payload)  # your existing helper
        if not text:
            raise HTTPException(status_code=400, detail="No text found in params.message.content")

        try:
            task_result = brain(text)  # returns a TASK RESULT dict (with meta)
